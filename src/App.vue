<template>
  <v-app>
    ...
    <v-content>
      <v-card class="mx-auto mt-6" max-width="600" outlined>
        <v-list-item three-line>
          <v-list-item-content>
            <v-list-item-title class="title mb-1"
              >使用 mathwallet 插件登录</v-list-item-title
            >
            <v-list-item-subtitle>{{
              account ? `${account.name}[${account.address}]` : ""
            }}</v-list-item-subtitle>
            <v-list-item-subtitle>{{
              account ? account.root_pub : ""
            }}</v-list-item-subtitle>
          </v-list-item-content>
        </v-list-item>
  ...
  methods: {
    ...
    */
    ...
    async transfer() {
      ...
      const publicKey = this.publicKeyByPath(
        this.account.xpub,
        derive_paths[0]
      );
      ...
      const nodeUrl = "https://lina.ckb.dev";
